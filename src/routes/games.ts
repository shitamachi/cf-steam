  ...
  try {
    ...
    const result = await db.transaction(async (tx) => {
      ...
      for (let i = 0; i < games.length; i += batchSize) {
        ...
        const batchResult = await tx.insert(schema.games)
          .values(batch.map(game => ({
            appid: game.appid,
            name: game.name,
            lastFetchedAt: game.lastFetchedAt ? new Date(game.lastFetchedAt * 1000) : new Date()
          })))
          .onConflictDoUpdate({
  ...
  try {
    ...
    const result = await db.insert(schema.games)
      .values({
        ...
        name: gameData.name,
        lastFetchedAt: gameData.lastFetchedAt ? new Date(gameData.lastFetchedAt * 1000) : new Date()
      })
      .onConflictDoUpdate({
        target: schema.games.appid,
        set: {
